...
def process_smd_dataset(data_path, output_path):
    ...
    # 合并特征和标签
    final_data = np.concatenate([test_data_scaled, test_labels_flipped], axis=1)
...
def process_smap_dataset(data_path, output_path):
    ...
    输出: smap_standardized.npz (仅测试数据)
    """
    # 加载数据
...
def process_swat_dataset(data_path, output_path):
    ...
    输出: swat_standardized.npz (仅测试数据)
    """
    # 加载数据
    ...
    for name, path in datasets.items():
        ...
        try:
            ...
            if name == "SMD":
                ...
                process_kpi_dataset(path, output_dir)  # 添加KPI
