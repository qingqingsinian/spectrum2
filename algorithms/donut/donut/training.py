...
class DonutTrainer(VarScopeObject):
    ...
    def __init__(self, model, model_vs=None, n_z=None,
                 ...
                 max_epoch=120, max_step=None, batch_size=256,
        ...
        with reopen_variable_scope(self.variable_scope):
            ...
            with tf.name_scope('loss'):
                ...
                if use_regularization_loss:
                    # TensorFlow 2.x compatibility: use tf.compat.v1.losses.get_regularization_loss()
                    if hasattr(tf.losses, 'get_regularization_loss'):
            ...
            with tf.name_scope('summary'):
                # TensorFlow 2.x compatibility: disable problematic summaries
                # Create a simple scalar summary instead of histogram summaries
                try:
                    self._summary_op = tf.summary.scalar('training_loss', self._loss)
                except Exception:
                    # If even scalar summary fails, create a no-op
    ...
    def fit(self, values, labels, missing, mean, std, excludes=None,
        ...
        with TrainLoop(
                ...
                max_step=self._max_step) as loop:  # type:
