...
# Global cache for Dense layers to avoid variable scope conflicts
_dense_layer_cache = {}

def softplus_std(inputs, units, epsilon, name):
    # Use cached Dense layer or create new one, but check graph compatibility
    cache_key = (units, name, id(tf.get_default_graph()))
    if cache_key not in _dense_layer_cache:
        _dense_layer_cache[cache_key] = keras.layers.Dense(units, name=name)
    dense_layer = _dense_layer_cache[cache_key]
    return tf.nn.softplus(dense_layer(inputs)) + epsilon
...
class Donut(VarScopeObject):
    ...
    def __init__(self, h_for_p_x, h_for_q_z, x_dims, z_dims, std_epsilon=1e-4,
        ...
        with reopen_variable_scope(self.variable_scope):
            # Create reusable Dense layers to avoid variable scope issues
            self._x_mean_layer = keras.layers.Dense(x_dims, name='x_mean')
            self._z_mean_layer = keras.layers.Dense(z_dims, name='z_mean')
