...
class Solver(object):
    ...
    def train(self):
        ...
        early_stopping = EarlyStopping(
            patience=5, verbose=True, dataset_name=self.data_path
        )
    ...
    def test(self):
        self.model.load_state_dict(
            torch.load(
                os.path.join(
                    str(self.model_save_path), str(self.data_path) + "_checkpoint.pth"
                )
            )
        )
        self.model.eval()
        ...
        fpr = fp / (fp + tn)
        print(
            "Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f}, FNR : {:0.4f}, FPR : {:0.4f} ".format(
                accuracy, precision, recall, f_score, fnr, fpr
            )
        )

        import csv

        csv_file_path = "../results/DCdetector.csv"
        ...
        with open(csv_file_path, "a", newline="") as f:
            ...
            # Write data row
            writer.writerow(
                [self.data_path, accuracy, precision, recall, f_score, fnr, fpr]
            )
