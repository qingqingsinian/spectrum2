...
def main(config):
    ...
    if config.mode == "train":
        ...
        start_time = time.time()
        solver.test()
        print("Total testing time: {}s".format(time.time() - start_time))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_multi_gpu", action="store_true", help="use multiple gpus", default=True
    )
    parser.add_argument(
    ...
    args = vars(config)
    config.patch_size = [int(patch_index) for patch_index in list(config.patch_size)]
    ...
    if not os.path.exists("../results"):
        os.makedirs("../results")
    sys.stdout = Logger("../results/" + config.data_path + ".log", sys.stdout)
    if config.mode == "train":
        print("\n\n")
