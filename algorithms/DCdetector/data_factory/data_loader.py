...
class UCRAUGSegLoader(object):
    def __init__(self, index, data_path, win_size, step, mode="train"):
        ...
        self.test_labels = np.load(
            data_path + "/UCR_AUG_" + str(index) + "_test_label.npy"
        )
        if self.mode == "val":
            print("train:", self.train.shape)
            print("test:", self.test.shape)
...
class SMD_OriSegLoader(object):
    def __init__(self, index, data_path, win_size, step, mode="train"):
        ...
        self.val = self.test
        self.test_labels = np.load(
            data_path + "/SMD_Ori_" + str(index) + "_test_label.npy"
        )
        if self.mode == "val":
            print("train:", self.train.shape)
            print("test:", self.test.shape)
...
def get_loader_segment(
    index, data_path, batch_size, win_size=128, step=100, mode="train", dataset="KDD"
):
    if dataset == "SMD":
