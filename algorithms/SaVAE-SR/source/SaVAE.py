...
class IntroVAE:
    ...
    def __init__(self, max_epoch=100, batch_size=256, latent_dims=3, window_size=120, cuda: bool=False, margin=15.0, print_fn=print):
        ...
        if self.cuda:
            ...
            self.beta = torch.tensor(1.0).to(self.device)
            self.margin = torch.tensor(margin).to(self.device)
            self.z_prior_dist = dist.Normal(
                torch.from_numpy(np.zeros((self.latent_dims,), np.float32)).to(self.device),
                torch.from_numpy(np.ones((self.latent_dims,), np.float32)).to(self.device))
            self.x_prior_dist = dist.Normal(
                torch.from_numpy(np.zeros((self.window_size,), np.float32)).to(self.device),
                torch.from_numpy(np.ones((self.window_size,), np.float32)).to(self.device))
        ...
        with Loop(max_epochs=self.max_epoch, use_cuda=self.cuda, disp_epoch_freq=5, print_fn=self.print_fn).with_context() as loop:
            ...
            for epoch in loop.iter_epochs():
                for _, batch_data in loop.iter_steps(train_dataloader):
                    ...
                    # 将数据移到正确的设备
                    if self.cuda:
                        observe_x = observe_x.cuda()
                        observe_normal = observe_normal.cuda()
                    else:
                        observe_x = observe_x.to(self.device)
                        observe_normal = observe_normal.to(self.device)
                ...
                if valid_kpi is not None:
                    with torch.no_grad():
                        for _, batch_data in loop.iter_steps(valid_dataloader):
                            ...
                            current_batch_size = observe_x.shape[0]  # 动态获取当前 batch 大小

                            p_x_z, p_z_x, observe_z = self.model(observe_x)
                            # Fake sample generated by the noise zp
                            zp = self.z_prior_dist.sample((current_batch_size,))
                            xp_mean, xp_std = self.model.generator(zp)
                            xp = self.reparameterization(xp_mean, xp_std)
                            # Fake sample generated by the noise z
                            xr = p_x_z.sample()
