...
class Silency(object):
    ...
    def transform_silency_map(self, values: np.ndarray) -> np.ndarray:
        """
    ...
    def transform_silency_map_phase(self, values: np.ndarray) -> np.ndarray:
        """
        Transform a time-series into spectral residual, which is method in computer vision.
    ...
    def transform_spectral_residual(self, values):
        ...
        spectral_residual = np.sqrt(silency_map.real ** 2 + silency_map.imag ** 2) # type: ignore
