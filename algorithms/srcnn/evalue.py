...
if __name__ == '__main__':
    ...
    with open(score_csv_filename, 'w', newline='') as csvfile:
        ...
        for flabel, scores, filename, ftimestamp in savedscore:
            ...
            for i in range(98):
                ...
                try:
                    fscore, _, _, _, _, _, _ = evaluate_for_all_series([[ftimestamp, flabel, preds, filename]], delay,
                                                                       prt=False)
            ...
            for ts, lbl, scr, pred in zip(ftimestamp, flabel, scores, predictions):
                writer.writerow({'file': os.path.basename(filename), 'timestamp': ts, 'label': lbl, 'score': scr,
                    'prediction': pred})
    ...
    with open(csv_filename, 'a', newline='') as csvfile:
        fieldnames = ['type', 'filename', 'f1_score', 'precision', 'recall', 'TP', 'FP', 'TN', 'FN', 'best_threshold',
                      'prediction_time', 'training_time', 'delay']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        # 写入总体结果
        writer.writerow(
            {'type': 'overall', 'filename': 'all_files', 'f1_score': total_fscore, 'precision': pre, 'recall': rec,
                'TP': TP, 'FP': FP, 'TN': TN, 'FN': FN, 'best_threshold': 'N/A', 'prediction_time': total_time,
        ...
        for f, (flabel, cnnscores, _, ftimestamp) in zip(files, savedscore):
            ...
            try:
                final_fscore, final_precision, final_recall, tp, fp, tn, fn = evaluate_for_all_series(file_result_final,
                                                                                                      delay, prt=False)
            except:
            ...
            # 写入单个文件的结果
            writer.writerow({'type': 'per_file', 'filename': os.path.basename(f), 'f1_score': final_fscore,
                'precision': final_precision, 'recall': final_recall, 'TP': tp, 'FP': fp, 'TN': tn, 'FN': fn,
                'best_threshold': bestthre, 'prediction_time': file_prediction_time, 'training_time': 'N/A',
                # 单个文件不涉及训练时间
                'delay': delay})

            print(
                f"File: {os.path.basename(f)} - Best F1: {final_fscore:.4f}, Precision: {final_precision:.4f}, Recall: {final_recall:.4f}, Best Threshold: {bestthre:.2f}, Prediction Time: {file_prediction_time:.4f}s")
...
def sr_cnn_eval(timestamp, value, label, window, net, ms_optioin, threshold=0.01, back_k=0, backaddnum=5, step=1):
    ...
    for pt in range(win_size - backaddnum + back + step, length - back, step):
        ...
        mag = SpectralResidual.spectral_residual(wave)
